...
class SupabaseProfileRepository implements IProfileRepository {
  ...
  Future<UserProfile> loadOrCreate() async {
    ...
    try {
      ...
      await client.from('users').upsert({'id': userId, 'email': email});
      final created = UserProfile.initial();
      await _box.put(_cacheKey, created);
  ...
  Future<void> save(UserProfile profile) async {
    ...
    try {
      ...
      // Don't rethrow here so offline mode works smoothly even if this fails
    }
  }
  ...
  UserProfile _profileFromRow(Map<String, dynamic> row) {
    ...
    return UserProfile(
      ...
      lastFreezeReset: lastFreeze != null
          ? DateTime.parse(lastFreeze as String)
          : null,
